...
router.get("/settings/branches", async (req, res) => {
  try {
    const { data, error } = await tenantSupabase(req).from("branches").select("*").order("name");
...
router.get("/settings/referral-providers", async (req, res) => {
  try {
    const { data, error } = await tenantSupabase(req).from("referral_providers").select("*").order("name");
    if (error) throw error;
    res.json(data ?? []);
  } catch (err) {
...
router.post("/settings/referral-providers", async (req, res) => {
  try {
    ...
    const { data: prov, error } = await tenantSupabase(req).from("referral_providers").insert({ name, specialty, phone, address }).select().single();
    if (error) throw error;
...
router.get("/settings/tax", async (req, res) => {
  try {
    let { data: settings } = await tenantSupabase(req).from("tax_settings").select("*").limit(1).single();
    if (!settings) {
      const { data: created } = await tenantSupabase(req).from("tax_settings").insert({}).select().single();
      settings = created;
    }
    res.json({
...
router.patch("/settings/tax", async (req, res) => {
  try {
    ...
    let { data: existing } = await tenantSupabase(req).from("tax_settings").select("id").limit(1).single();
    if (!existing) {
      const { data: created } = await tenantSupabase(req).from("tax_settings").insert({}).select().single();
      existing = created;
    }
    const { data: updated, error } = await tenantSupabase(req).from("tax_settings").update(updates).eq("id", (existing as any).id).select().single();
    if (error) throw error;
    res.json({
...
router.get("/settings/system", async (req, res) => {
  try {
    let { data: settings } = await tenantSupabase(req).from("system_settings").select("*").limit(1).single();
    if (!settings) {
      const { data: created } = await tenantSupabase(req).from("system_settings").insert({}).select().single();
      settings = created;
    }
    res.json({
...
router.patch("/settings/system", async (req, res) => {
  try {
    ...
    let { data: existing } = await tenantSupabase(req).from("system_settings").select("id").limit(1).single();
    if (!existing) {
      const { data: created } = await tenantSupabase(req).from("system_settings").insert({}).select().single();
      existing = created;
    }
    const { data: updated, error } = await tenantSupabase(req).from("system_settings").update(updates).eq("id", (existing as any).id).select().single();
    if (error) throw error;
    res.json({
...
router.get("/settings/working-days", async (req, res) => {
  try {
    const { data, error } = await tenantSupabase(req).from("working_days").select("*").order("branch").order("day_of_week");
    if (error) throw error;
    res.json(data ?? []);
  } catch (err) {
...
router.put("/settings/working-days", async (req, res) => {
  try {
    ...
    const { data: existing } = await tenantSupabase(req).from("working_days").select("*");
    const results = await Promise.all(days.map(async (d) => {
      const found = (existing ?? []).find((r: any) => r.branch === d.branch && r.day_of_week === d.dayOfWeek);
      if (found) {
        const { data: updated } = await tenantSupabase(req).from("working_days").update({ is_working: d.isWorking, open_time: d.openTime ?? null, close_time: d.closeTime ?? null }).eq("id", (found as any).id).select().single();
        return updated;
      } else {
        const { data: created } = await tenantSupabase(req).from("working_days").insert({ branch: d.branch, day_of_week: d.dayOfWeek, is_working: d.isWorking, open_time: d.openTime ?? null, close_time: d.closeTime ?? null }).select().single();
        return created;
      }
    }));
    res.json(results);
...
router.get("/settings/holidays", async (req, res) => {
  try {
    const { data, error } = await tenantSupabase(req).from("holidays").select("*").order("date");
    if (error) throw error;
    res.json(data ?? []);
  } catch (err) {
...
router.post("/settings/holidays", async (req, res) => {
  try {
    ...
    const { data: row, error } = await tenantSupabase(req).from("holidays").insert({ branch: branch ?? null, date, title }).select().single();
    if (error) throw error;
...
router.delete("/settings/holidays/:id", async (req, res) => {
  try {
    ...
    const { error } = await tenantSupabase(req).from("holidays").delete().eq("id", id);
    if (error) throw error;
    res.status(204).end();
  } catch (err) {
...
router.delete("/danger/clear-appointments", async (req, res) => {
  try {
    const { error } = await tenantSupabase(req).from("appointments").delete().neq("id", 0);
    if (error) throw error;
...
router.delete("/danger/clear-visits", async (req, res) => {
  try {
    const { error } = await tenantSupabase(req).from("visits").delete().neq("id", 0);
    if (error) throw error;
